...
from visualization.joints2smpl.src import config
    ...
    def __call__(self, init_pose, init_betas, init_cam_t, j3d, conf_3d=1.0, seq_ind=0):
       ...
       # -------------Step 1: Optimize camera translation and body orientation--------
        ...
        if self.use_lbfgs:
            ...
            for i in tqdm.tqdm(range(10), desc='training smpl camera ...'):
                def closure():
            ...
            for i in tqdm.tqdm(range(20), desc='training smpl camera ...'):
                smpl_output = self.smpl(global_orient=global_orient,
                                        body_pose=body_pose,
                                        betas=betas)
                model_joints = smpl_output.joints
        ...
        if self.use_lbfgs:
            body_optimizer = torch.optim.LBFGS(body_opt_params, max_iter=self.num_iters,
                                               lr=self.step_size, line_search_fn='strong_wolfe')
            for i in tqdm.tqdm(range(self.num_iters), desc='training smpl body ...'):
                def closure():
