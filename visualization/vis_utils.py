...
class npy2obj:
    def __init__(self, npy_path, sample_idx, rep_idx, device=0, cuda=True, n_iter=2):
        ...
        if self.motions.ndim == 3:
            m = torch.from_numpy(self.motions).permute(1, 2, 0).unsqueeze(0).numpy()
            self.motions = {}
            self.motions.update({'motion': m})
            self.motions.update({'text': ['']})
            self.motions.update({'num_samples': 1})
            self.motions.update({'lengths': [m.shape[-1]]})
        ...
        self.j2s = joints2smpl(num_frames=self.num_frames, device=device, n_iter=n_iter)
